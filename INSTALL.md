...
```
schtasks /create /tn MySQLConsole /sc onstart /ru SYSTEM /tr "\"E:\code\mysql-console\.venv\Scripts\pythonw.exe\" \"E:\code\mysql-console\server.py\""
```
...
```bash
sudo ./install.sh --service      # 注册 + 开机自启 + 立即启动
...
**Q2:8090 端口被占用?**
临时换端口:Linux `MC_PORT=9090 ./start.sh`(start.sh 支持端口参数),或改 `server.py` 底部 `PORT`;Windows 临时改法同(编辑 server.py)。
