...
| 模块 | 行数 | 职责 |
|---|---|---|
| [backup_engine.py](../../src/backup_engine.py) | 1381 | **备份/还原引擎**。`run_backup` 双层参数签名（`conn_cfg` 隧道化端点 vs `storage_cfg` 原始 host 判定存储位置）；`_dump_to_file`/`_dump_to_remote` 流式管道（1MB 分块 + 双 stderr 线程解析表切换与错误）；gzip level=6；内存 `TASKS` 任务管理器 + task_id 轮询（done/failed 保留最近 100 条，终态即清理）；**`_task_lock` 全局互斥**（`start_*_task` 非阻塞 acquire，占用返回 None→API 409；调度经 `task_busy()`/`try_acquire_task_lock()` 纳入同一互斥）；子进程密码走 `MYSQL_PWD` 环境变量（`_child_env`），不落 argv；restore 含建库语句识别（前 256KB）、zip 成员 basename 防穿越、远程 zip 拒绝；备份文件浏览与下载白名单校验；历史记录双后端落库 |
...
| [native_script.py](../../src/native_script.py) | 433 | **自包含备份脚本生成器**（计划任务只调脚本不经 Python）。Windows `.ps1`（UTF-8 BOM+CRLF，生成后 icacls 收紧 ACL）/ Linux `.sh`（chmod 700）；脚本内：探测 mysqldump（配置目录→PATH）→ mysqldump → 校验（MIN_SIZE 防空包）→ gzip → keep 清理 → 日志；密码明文内嵌（单引号转义；单机工具取舍，详见 01-architecture §11） |
...
| [mysql_installer.py](../../src/mysql_installer.py) | ~1010 | **本机数据库检测 + MySQL Server 免安装版部署编排**（2026-09 新增）。`detect_local_server()` 三分量检测（OS 服务/mysqld 二进制/3306 端口 MySQL 握手，任一命中即 installed）；`fetch_official_versions()` 版本枚举（endoflife API → cdn HEAD 逐链验证 → 内置兜底清单，24h 缓存）；`build_config_suggestions()` 参数建议（buffer pool/连接数按内存分档、排序规则按大版本）；`render_my_cnf()` 按版本分支渲染（redo capacity 新旧参数名/lower_case 初始化前锁定）；`_install_worker()` 六阶段编排（多镜像下载→防穿越解压（官方包合法符号链接放行/逃逸链接拒绝）→my.ini→initialize-insecure→进程启动→root 密码 Fernet 链路）+ 可选服务注册（sc/systemd，权限不足优雅降级）；CLI：`--detect-only`（install 脚本挂钩）/`--install-json`（真机调试通道，API 白名单不透传 url_override） |
